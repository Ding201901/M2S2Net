...
def grid_search(args, num):
    param_grid = {
        ...
        'depth': [4],
        'head': [4],
        'seed': [2031, 2032, 2033, 2035, 2036],  # 2030, 2031, 2032, 2033, 2034, 2035
    ...
    # parameters = list(ParameterSampler(param_grid, n_iter = 200, random_state = 0))[num]
    ...
    args.learning_rate = parameters['lr']
    args.dim = parameters['dim']
...
if __name__ == '__main__':
    ...
    results = []  # 保存训练结果
    max_search = 5
    args.dataset = 'Yellow River'  # Yellow River  Bay_Area
    for i in range(0, max_search):
        ...
        np.savetxt(f'{args.dataset}-{args.flag_test}.txt', np.array(results),
                   fmt = "%i, %i, %i, %i, %.4f, %i, %.2f, %i, %i, %.6f, %.6f, %.6f")  # %.2f,
        #              seed,ratio,patches,lr,head,dim,depth,batch_size, num, oa, aa, kappa
        #              batch_size,depth,dim,head,lr,patches,ratio,seed,NO,OA,AA,Kappa
